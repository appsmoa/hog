          ...
          try {
            ...
            window.changeAptDealMonth = async (aptcd, guCode, baseYmd, diff) => {
              ...
              try {
                ...
                // 로딩 상태 표시
                const tableBody = document.querySelector('.apt-info-overlay table tbody');
                if (tableBody) {
                  tableBody.innerHTML = '<tr><td colspan="4" align="center" style="padding:20px;">로딩중...</td></tr>';
                }
                // 데이터 로드 후 테이블만 업데이트
                if (window[`showAptInfo_${aptcd}`]) {
                  await window[`showAptInfo_${aptcd}`](newYmd);
                }
              } catch (error) {
                console.error('월 이동 중 오류 발생:', error);
              }
            };
            ...
            const infoContent = `
              ...
              <div style="margin-top:16px;">
                ...
                <button onclick="event.preventDefault(); window.changeAptDealMonth('${apart.aptcd}','${guCode}','${dealYmd}',-1)" style="background:none;border:none;font-size:18px;cursor:pointer;color:#3490dc;padding:0 6px;">&#8592;</button>
